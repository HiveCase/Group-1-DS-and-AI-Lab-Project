...
**`.env.example` exists at the repo root, but is stale**: it also lists `JWT_SECRET_KEY`, `JWT_ACCESS_TOKEN_EXPIRE_MINUTES`, `JWT_REFRESH_TOKEN_EXPIRE_DAYS`, `MAX_UPLOAD_SIZE_MB`, `CHROMA_PERSIST_DIR`, `LLM_PROVIDER`, `USE_LANGGRAPH`, `MOCK_LLM_FAIL_AGENT`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `OLLAMA_MODEL`, and `OLLAMA_BASE_URL` — **none of these are read anywhere in `backend/app`** (verified by grep against the `Settings` class and the rest of the codebase). They appear to be leftovers from an earlier design (JWT auth, Ollama as the LLM provider) that was never built or was since replaced. TODO: fill in manually — confirm with the team whether to prune `.env.example` down to the variables in the table above, or whether some of these map to planned-but-unbuilt features.
...
├── specs/001-claims-portal/         # spec-kit: spec.md, plan.md, data-model.md, tasks.md, research.md
├── docs/gke-cicd.md                 # GKE deployment runbook
...
- **No authentication or role-based access control** — all four portals are open routes; anyone can act as any role.
