...
This project set out to automate the initial, preliminary stage of vehicle-insurance damage assessment — not to replace human adjusters — and delivered a working, five-agent, LangGraph-coordinated pipeline that detects damage (validation mask mAP50 0.449 in aggregate — but 0.28–0.32 specifically for `dent`/`scratch`/`crack`, this application's most common damage types, §7.1), retrieves policy-grounded coverage clauses (mean P@3 0.9133 by deterministic tag-matching, though the LLM-judged faithfulness/answer-correctness figures are considerably weaker at 0.63/0.52, §7.3), generates a structured recommendation, and screens for fraud signals (a hand-tuned rule engine, not validated against real fraud outcomes — README.md §6.5), all served through a four-portal web application with reproducible local and containerized deployment paths (no live production deployment has been demonstrated). The project's own most consistent, cross-validated finding — that `dent`/`scratch`/`crack` detection difficulty is an intrinsic property of those damage types' low visual distinguishability, not an artifact of dataset size or training configuration, reproducing identically across two independently trained model tracks (Figure 5) — is arguably as valuable a result as the headline metrics themselves, since it directly informs where future effort should (and should not) go.
...
2. **Build a quantitative end-to-end evaluation** — a small, manually-verified test set covering representative claim scenarios run through the full image → detection → severity → retrieval → report → recommendation chain, and a systematic (not n=5) robustness benchmark. The per-stage numbers in §7 are real, but nothing currently measures the composed pipeline's end-to-end correctness against a known-good answer key.
3. **Model per-user claim ownership.** Backend RBAC now gates *which portals* a role can reach (`user` vs. `admin` — see the `users` table note in README.md §1), but there's no foreign key from `Claim` to `users` yet, so any logged-in `user` can still look up any claim by ID. Scoping claim visibility to the submitting account is the natural next step.
...
---
**Demo Video :** [Click here to watch the video on Google Drive](https://drive.google.com/file/d/1GixhfgLSFee5t9_m8pY_c1VvJ23hkR1j/view?usp=sharing)
