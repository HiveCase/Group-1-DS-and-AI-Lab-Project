...
**One caveat carried through this section:** the confusion matrix and precision-recall curves in Section 8 are read directly from files Ultralytics writes into a run's own output folder, which an unrelated later `.val()` call against the same weights can silently overwrite if it isn't given its own distinct output location. An earlier draft of this report was caught by exactly that: both artefacts were reflecting the **validation** split, not the test split, because the folder they were read from was last written by the Tuned run's own end-of-training validation pass. **This has since been corrected for the precision-recall curves** (Section 8.3) — they are now regenerated by a dedicated `.val(split="test", project=..., name=..., exist_ok=True)` call that writes to its own named folder, immune to being overwritten by any other `.val()` call in the pipeline, and their legend mAP values match this report's test-split numeric tables (Section 5.2) exactly. **The confusion matrix (Section 8.2) still has the old validation-split mismatch** and is flagged as an open item in Section 11.
...
**These curves are computed directly on the held-out test split** — regenerated by a dedicated `model.val(split="test", ...)` call saved to its own named output folder (see Section 3.3), not read from the Tuned run's training-time validation artefacts as in an earlier draft of this report. The per-class mAP values in each legend match this report's test-split per-class table (Section 5.2) exactly: `shattered_glass 0.819` box / `0.775` mask, `broken_lamp 0.600` / `0.476`, `flat_tyre 0.550` / `0.529`, `scratch 0.344` / `0.288`, `crack 0.344` / `0.328`, `dent 0.290` / `0.290`.

Both curves visually confirm the class ranking established in Section 8.1: `shattered_glass` (purple) sits well above every other class across nearly the full recall range, with precision staying above 0.8 out to roughly 0.7-0.75 recall before dropping sharply. `broken_lamp` (red) and `flat_tyre` (brown) form a middle tier, trading the lead with each other at different points along the curve rather than one cleanly dominating the other. `dent` (blue) is the single weakest class on both curves, with `scratch` (orange) and `crack` (green) clustered just above it — on the Box curve `scratch` and `crack` are tied closely together (0.344 each) and clearly separated from `dent`; on the Mask curve it is `dent` and `scratch` that sit nearly on top of each other at the bottom (0.290 vs. 0.288), with `crack` (0.328) a little above both.
...
**Diagnostic-plot split mismatch — partially resolved.** As noted in Section 3.3, the confusion matrix and PR curves (Section 8.2-8.3) were both originally read from a run folder last written by the Tuned model's training-time validation pass, so both reflected the validation split rather than the test split. **The PR curves (Section 8.3) are now fixed** — regenerated by a `.val(split="test", project=..., name=..., exist_ok=True)` call that saves to its own dedicated folder, so it can never be silently overwritten by another `.val()` call later in the same notebook (the exact failure mode that caused this in the first place). **The confusion matrix (Section 8.2) has not yet been regenerated the same way** and still reflects the validation split; the fix is the identical, small, mechanical change (a dedicated `project=`/`name=` on a `split="test"` `.val()` call) and remains open work.
...
- **Fix the confusion matrix split mismatch** (Section 11) - the PR curves have already been corrected against the test split; the same small, mechanical fix (a dedicated `project=`/`name=` on a `split="test"` `.val()` call) still needs to be applied to the confusion matrix to make every diagnostic plot in this report consistent with the test-split numeric tables.
